...
int main(void)
{
    ...
    USART_ITConfig(EVAL_COM1, USART_IT_TXE, ENABLE); /// 缓冲区+允许中断
    ...
    while (USART_GetFlagStatus(EVAL_COM1, USART_FLAG_TC) == RESET) /// 暴力循环等待2（没有OS）
    {}

    /* Enable the EVAL_COM1 Receive interrupt: this interrupt is generated when the
         EVAL_COM1 receive data register is not empty */
    USART_ITConfig(EVAL_COM1, USART_IT_RXNE, ENABLE); /// 缓冲区+允许中断
